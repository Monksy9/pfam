...
class EmbedderTransformer():
    """
    Simple process wrapper for the various embedders. 
    
    Appies Embedders to amino acid input sequences containing .embed_input_sequences and unload their output locally.
    """

    def __init__(self, train_size, val_size, test_size, num_gpus:int, sequence_length:int = 512):    
        Loader = ld.DataLoader()
        self.train_df, self.val_df, self.test_df = Loader.load_all_three_dfs(train_size=train_size, val_size=val_size, test_size=test_size)
        self.sequence_length = sequence_length
        self.num_gpus = num_gpus
    
    def embed_input_sequences(self, embedder_name):
        
        if 'esm1' in embedder_name or 'protbert' in embedder_name:
            print('Using bio-transformers embedder')
            embedder = pe.PretrainedEmbedder(model_name=embedder_name, sequence_length=self.sequence_length, num_gpus=self.num_gpus)
        ...
        train_embeddings_df = self._append_embedder_name_to_columns(train_embeddings_df, embedder_name=embedder_name)
        test_embeddings_df = self._append_embedder_name_to_columns(test_embeddings_df, embedder_name=embedder_name)
        val_embeddings_df = self._append_embedder_name_to_columns(val_embeddings_df, embedder_name=embedder_name)
    ...
    def _append_embedder_name_to_columns(self, df: pd.DataFrame, embedder_name:str) -> pd.DataFrame:
        
        df.columns = [embedder_name[0:5] + str(col) for col in df.columns if col not in ['family_id', 'sequence']]
